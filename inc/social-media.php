...
echo '<ul class="nav-social list-inline navbar-right">';
	// Loop through array and output the item only if the field is not empty
	foreach ( $profiles as $profile => $class ) {
		if ( !empty ( $ft_option[$profile] )) { echo '<li><a href="' . esc_url( $ft_option[$profile] ) . '" target="_blank">
			<i class="fa fa-'.$class.'"></i></a>
			</li>'; }
	}
echo '</ul>';
?>